...
#: Maximum interval (seconds) between transmit-queue flushes performed by the
#: service thread.  Bounds the latency of messages queued by other threads (e.g.
#: command responses produced by the receive/Notifier thread) even when no cyclic
#: timer is due.  Cyclic frames are still fired at their exact deadlines.
_TX_FLUSH_INTERVAL_S = 0.01
...
class ElectronicControlUnit:
    ...
    def __init__(self, data_link_layer='j1939-21', max_cmdt_packets=1, minimum_tp_rts_cts_dt_interval=None, minimum_tp_bam_dt_interval=None, send_message=None):
        ...
        # Outgoing frames are queued here and transmitted by the single service
        # thread (see :meth:`_timer_job_thread`), so that no application or
        # receive-thread code ever blocks on ``bus.send()``.  Transport-protocol
        # (TP/BAM) continuation frames are the one exception: they are produced by
        # the protocol thread and sent on demand (see :meth:`send_message`).
    ...
    def send_message(self, can_id, extended_id, data, fd_format=False):
        """Queue a raw CAN message for transmission.
        ...
        The message is placed on the transmit queue and actually put on the bus
        by the single service thread (:meth:`_timer_job_thread`).  Deferring the
        ``bus.send()`` keeps it off the caller's thread — in particular off the
        receive (Notifier) thread, whose message handlers must return promptly so
        the kernel socket keeps being drained.  The sole exception is
        transport-protocol continuation frames, which are generated by the
        protocol thread and sent on demand so multi-frame transfers are not
        rate-limited to the service thread's flush cadence.
        ...
        # TP/BAM continuation frames come from the protocol thread; send them
        # immediately (on demand).  Everything else is queued and flushed by the
        # service thread so no transmit runs on the receive/application threads.
        if threading.current_thread() is self._protocol_thread:
            self._transmit(msg)
        else:
    ...
    def _transmit(self, msg):
        """Put a single message on the bus, serialized against other senders.

        :param can.Message msg: the message to transmit.
        """
        with self._send_lock:
            self._bus.send(msg)

    def _drain_tx_queue(self):
        """Transmit every message currently queued for sending.

        Called by the service thread each pass.  A failure to transmit one
        message is logged and does not stop the rest of the queue from draining.
        """
        if not self._bus:
            return
        while True:
            ...
            try:
                ...
                logger.exception("Failed to transmit queued CAN message")
    ...
    def _timer_job_thread(self):
        """Service thread: fires application cyclic callbacks and flushes TX.

        Uses a heapq (min-heap keyed by deadline) for O(log n) scheduling of
        cyclic callbacks, and is the single owner of ``bus.send()`` for all
        non-TP traffic.  Each pass it fires every due timer callback (which queue
        their frames) and then drains the transmit queue onto the bus.  It sleeps
        until the next timer deadline, but never longer than
        ``_TX_FLUSH_INTERVAL_S`` so frames queued by other threads (e.g. command
        responses from the receive thread) are transmitted promptly.  Woken early
        via _timer_wakeup_queue whenever a timer is added/removed.  Callbacks
        returning True are rescheduled; returning False are removed.
        """
        while not self._job_thread_end.is_set():
            now = time.monotonic()
            next_wakeup = now + 5.0
            ...
            # Transmit everything the timer callbacks (and other threads) queued.
            self._drain_tx_queue()

            # Never sleep longer than the flush interval, so messages queued by
            # other threads are put on the bus without waiting for a cyclic timer.
            next_wakeup = min(next_wakeup, time.monotonic() + _TX_FLUSH_INTERVAL_S)
